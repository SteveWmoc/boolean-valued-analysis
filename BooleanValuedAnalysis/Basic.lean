...
def Index : BVSet 𝔹 → Type u
  | .mk ι _ _ => ι

/-- The child at a given index. -/
def child : (x : BVSet 𝔹) → x.Index → BVSet 𝔹
  | .mk _ A _, i => A i

/-- The Boolean coefficient attached to a child. -/
def weight : (x : BVSet 𝔹) → x.Index → 𝔹
  | .mk _ _ w, i => w i

@[simp]
theorem mk_index (ι : Type u) (A : ι → BVSet 𝔹) (w : ι → 𝔹) :
    (BVSet.mk ι A w).Index = ι :=
  rfl

@[simp]
theorem mk_child (ι : Type u) (A : ι → BVSet 𝔹) (w : ι → 𝔹) (i : ι) :
    (BVSet.mk ι A w).child i = A i :=
  rfl

@[simp]
theorem mk_weight (ι : Type u) (A : ι → BVSet 𝔹) (w : ι → 𝔹) (i : ι) :
    (BVSet.mk ι A w).weight i = w i :=
  rfl

@[simp]
theorem eta (x : BVSet 𝔹) :
...
instance : EmptyCollection (BVSet 𝔹) :=
  ⟨empty⟩

/-- A singleton whose unique element has coefficient `b`. -/
def singleton (x : BVSet 𝔹) (b : 𝔹) : BVSet 𝔹 :=
